...
def get_pokemon_data(pid):
    """Busca os dados de um Pokémon específico, focado no FRLG."""
    # Ignora Pokémon da Geração 4 em diante (ID > 386)
    if pid > 386:
        return None
    ...
    for item in data['moves']:
        for v in item['version_group_details']:
            # MUDANÇA: Buscando agora na versão firered-leafgreen
            if v['version_group']['name'] == 'firered-leafgreen' and v['move_learn_method']['name'] == 'level-up':
...
def gerar_planilha_horizontal():
    print("Iniciando a montagem da planilha horizontal pelas cadeias evolutivas (FRLG)...")
    print("Isso vai levar um tempo (cerca de 5 a 10 minutos) pois a API tem limites de velocidade.\n")
    
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Familias FRLG"
    ...
    # Gen 3 (até Deoxys) está contida nas primeiras ~215 evolution chains, mas deixei margem até 260
    for chain_id in range(1, 260):
        ...
        for path_ids in paths:
            ...
            for pid in path_ids:
                ...
                if data: # Só adiciona se for Gen 3 ou anterior (ID <= 386)
