...
# Novo dicionário para os métodos
METODOS_EMOJI = {
    'level-up': '📈 Nível',
    'machine': '💿 TM/HM',
    'egg': '🥚 Ovo',
...
def buscar_golpes_linhagem_frlg(pokemon_id):
    ...
    for i, (nome_golpe, metodos) in enumerate(golpes_dict.items(), 1):
        ...
        try:
            ...
            # Aplicando os emojis nos métodos de aprendizagem
    ...
    # 5. Impressão
    print("\n" + "="*112)
    # Coluna MÉTODO(S) aumentada para 45 para caber os emojis direitinho
    print(f"{'NOME DO GOLPE':<18} | {'TIPO':<11} | {'MÉTODO(S)':<45} | {'FORÇA':<6} | {'ACURÁCIA':<8} | {'PP':<4}")
    print("="*112)
    ...
    for g in lista_final:
        ...
        print(f"{g['nome']:<18} | {tipo_formatado} | {g['metodos']:<45} | {forca_exibicao:<6} | {g['accuracy']:<8} | {g['pp']:<4}")
    print("="*112)

if __name__ == "__main__":
    print("Bem-vindo ao Analisador de Movesets FireRed/LeafGreen (Versão Emoji)!")
    while True:
