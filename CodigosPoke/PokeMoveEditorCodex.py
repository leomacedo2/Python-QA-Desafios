...
class PokemonEditor:
    def __init__(self, root):
        ...
        self.alvo_somente_familia_var = tk.BooleanVar(value=False)

        self.chk_alvo_familia = ctk.CTkCheckBox(
        self.frame_alvo,
            text="Somente família atual",
            variable=self.alvo_somente_familia_var,
            command=self.filtrar_lista_alvo,
            fg_color="#b8860b"
        )
        self.chk_alvo_familia.pack(fill="x", padx=10, pady=5)
    ...
    def aplicar_excecoes_familia(self, familia):
        familia_set = set(familia)
        ...
        if familia_set.intersection(familia_burmy):
            return ["burmy", "wormadam-plant", "wormadam-sandy", "wormadam-trash", "mothim"]
        ...
        if familia_set.intersection(familia_shaymin):
            return ["shaymin-land", "shaymin-sky"]
        ...
        if familia_set.intersection(familia_rotom):
            return ["rotom", "rotom-heat", "rotom-wash", "rotom-frost", "rotom-fan", "rotom-mow"]
        ...
        if familia_set.intersection(familia_giratina):
            return ["giratina-altered", "giratina-origin"]
        ...
        if familia_set.intersection(familia_deoxys):
    ...
    def filtrar_lista_alvo(self, *args):
        ...
        for item in self.lista_formatada:
            ...
            if hasattr(self, 'alvo_somente_familia_var') and self.alvo_somente_familia_var.get():
                nomes_familia = self.obter_nomes_csv_familia_atual()
                if pname not in nomes_familia:
                    continue
    ...
    def obter_nome_csv_por_nome_api(self, nome_api):
        ...
        for nome_csv in self.df_original['Name'].astype(str).tolist():
            if self.traduzir_nome_para_api(nome_csv) == nome_api:
                return nome_csv

        return None

    def obter_nomes_csv_familia_atual(self):
        if not hasattr(self, 'familia_atual_api_nomes'):
            return []

        nomes = []
        for nome_api in self.familia_atual_api_nomes:
            nome_csv = self.obter_nome_csv_por_nome_api(nome_api)
            if nome_csv:
                nomes.append(nome_csv)

        return nomes

    def obter_golpes_do_pokemon(self, nome_pokemon):
        if self.df_original is None:
            return set()

        linhas = self.df_original[self.df_original['Name'] == nome_pokemon]
        if linhas.empty:
            return set()
    ...
    def atualizar_alertas_copia_familia(self):
        ...
        if not nomes_familia:
            for frame in self.obter_frames_linhas():
                if hasattr(frame, 'btn_copiar'):
                    frame.btn_copiar.configure(border_width=0)

                if hasattr(frame, 'tooltip_copiar'):
                    frame.tooltip_copiar.set_text("")
            return

        for frame in self.obter_frames_linhas():
            if not hasattr(frame, 'btn_copiar') or not hasattr(frame, 'move_entry'):
                continue

            move = frame.move_entry.get().strip()

            if move in ["", "0", "NOVO_GOLPE"]:
                frame.btn_copiar.configure(border_width=0)
                continue
            ...
            for nome_familia in nomes_familia:
                if nome_familia == self.pokemon_atual:
                    continue

                golpes = self.obter_golpes_do_pokemon(nome_familia)
                if move not in golpes:
                    faltando.append(nome_familia)

            if faltando:
                frame.btn_copiar.configure(border_width=2, border_color="#FFD700")
                frame.tooltip_copiar.set_text("Falta em:\n" + "\n".join(faltando))
            else:
                frame.btn_copiar.configure(border_width=0)
                frame.tooltip_copiar.set_text("")
    ...
    def criar_linha_ui(self, move, lvl):
        ...
        btn_copiar = ctk.CTkButton(
            ...
            text="📋",
            width=30,
            command=lambda m=m_entry, l=l_entry: self.iniciar_copia(m.get(), l.get())
        )
        btn_copiar.pack(side="left", padx=2)

        tooltip = Tooltip(btn_copiar)
        btn_copiar.bind("<Enter>", tooltip.show)
        btn_copiar.bind("<Leave>", tooltip.hide)

        f.btn_copiar = btn_copiar
        f.move_entry = m_entry
        f.tooltip_copiar = tooltip
    ...
    def iniciar_copia(self, move, lvl):
        ...
        if hasattr(self, 'alvo_somente_familia_var'):
            self.alvo_somente_familia_var.set(True)
        self.filtrar_lista_alvo()
